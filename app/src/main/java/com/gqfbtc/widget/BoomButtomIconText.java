...
public class BoomButtomIconText extends FrameLayout {
    ...
    public BoomButtomIconText(Context context) {
        super(context);
        initView(context);
    }

    public BoomButtomIconText(Context context, AttributeSet attrs) {
        super(context, attrs);
        initView(context);
    }

    public BoomButtomIconText(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void show() {
        ...
        if (boomBtn.size() != 0 && entities != null) {
            for (int i = 0; i < entities.size(); i++) {
                ...
                TextView boom_bottom_title = (TextView) boomBtn.get(i).findViewById(R.id.boom_bottom_title);
                ...
                bgImg.setImageResource(entities.get(i).getBgIcon());
                ...
                boom_title.setVisibility(GONE);
                boom_subtitle.setVisibility(GONE);
                boom_icon.setVisibility(GONE);
                boom_bottom_title.setVisibility(VISIBLE);


                boom_title.setText(entities.get(i).getmTitle());
                boom_bottom_title.setText(entities.get(i).getmTitle());
                boom_icon.setText(CommonUtils.getString(entities.get(i).getmIcon()));

                ((LayoutParams) bgImg.getLayoutParams()).setMargins(entities.get(i).getPadding(), entities.get(i).getPadding(), entities.get(i).getPadding(), entities.get(i).getPadding());
                ...
                //设置宽高
                LayoutParams layoutParams = (LayoutParams) boomBtn.get(i).getLayoutParams();
    ...
    public static class BoomBtnEntity {
        ...
        public BoomBtnEntity setBgIcon(int bgIcon) {
            this.bgIcon = bgIcon;
            return this;
        }
