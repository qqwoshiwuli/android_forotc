...
public class ProgressAdvertisingFragment extends BasePullFragment<BaseFragentPullDelegate, BaseFragmentPullBinder> {
            ...
            public void onShelvesClick(final int position) {
                UiHeplUtils.initDefaultDialog(getActivity(), CommonUtils.getString(R.string.str_warning_isshelves_advertising), new View.OnClickListener() {
                    ...
                    public void onClick(View view) {
                        addRequest(binder.down(adapter.getDatas().get(position).getId(), ProgressAdvertisingFragment.this));
                    }
                }).show();
            }
    ...
    private void initTimeDialog(final int posotion) {
        new TimeChooseDialog(getActivity(), new TimeChooseDialog.OnTimeChooseLinsener() {
            ...
            public void setOnTimeChooseListener(String day1, String time1, String day2, String time2) {
                addRequest(binder.changetime(adapter.getDatas().get(posotion).getId(), day1 + "_" + time1, day2 + "_" + time2, ProgressAdvertisingFragment.this));
            }
        }).show();
    }
